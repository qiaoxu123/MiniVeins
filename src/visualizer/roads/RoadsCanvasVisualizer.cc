...
#include "src/visualizer/roads/RoadsCanvasVisualizer.h"
#include "src/modules/mobility/traci/TraCIScenarioManager.h"
#include "src/modules/mobility/traci/TraCICommandInterface.h"

using test::RoadsCanvasVisualizer;

Define_Module(test::RoadsCanvasVisualizer);
...
void RoadsCanvasVisualizer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        auto onTraciInitialized = [this, manager](test::SignalPayload<bool> payload) {
...
cPolylineFigure* RoadsCanvasVisualizer::createLine(const std::list<test::Coord>& coords, cFigure::Color color, double width, bool zoom)
{
