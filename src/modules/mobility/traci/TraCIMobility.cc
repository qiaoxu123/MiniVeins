...
using test::TraCIMobility;

Define_Module(test::TraCIMobility);
...
void TraCIMobility::changePosition()
{
    ...
    if (std::string(hostMod->getDisplayString().getTagArg("src", 0)) == ". ") {
        hostMod->getDisplayString().setTagArg("src", 0, " .");
    }
    else {
        hostMod->getDisplayString().setTagArg("src", 0, ". ");
    }
