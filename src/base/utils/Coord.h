//
// Copyright (C) 2004 Telecommunication Networks Group (TKN) at Technische
...
#include "src/base/utils/FWMath.h"
...
public:
     ...
     * Takes two equal-length sequences of numbers (usually coordinate vectors)
     * and returns a single number. in Euclidean geometry: the dot product of the
     * Cartesian coordinates of two vectors (i.e. inner product).
     */
    ...
    /**
     * @brief Returns the magnitude of the vector that would result from a regular
     * 3D cross product of the input vectors; The values on the z-plane are
     * assumed to be 0 (i.e. treating the 2D space as a plane in the 3D space).
     * The 3D cross product will be perpendicular to that plane, and thus have 0 X
     * & Y components (thus the scalar returned is the Z value of the 3D cross
     * product vector).
     */
     ...
     *
     * Because coordinates are floating point values, this is done using an
     * epsilon comparison.
    ...
    /**
     * @brief Returns the squared distance on a torus of this to Coord 'b' (omits
     * calling square root).
     */
    ...
    {
        return Coord(this->x < a.x ? this->x : a.x, this->y < a.y ? this->y : a.y,
